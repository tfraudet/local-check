...
- **No feasible route → straight line**: when terrain leaves no valid detour, routing returns nothing and the panel falls back to the straight-line profile, labelled *Straight glide*. This is deliberate: the chart then shows the glide plane crossing the terrain, which is the honest answer, rather than a *Routed glide* whose path clips rock.
