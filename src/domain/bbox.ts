...
/**
 * Expand a bbox proportionally on every side.
 * @param bbox The original bounding box [minLon, minLat, maxLon, maxLat]
 * @param factor The fraction by which to extend each side (e.g., 0.10 for 10% on each side)
 */
export function bufferBboxProportionally(bbox: Bbox, factor: number): Bbox {
  ...
  // Calculate degree padding based on the box's own size
